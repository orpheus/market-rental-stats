...
async function getMarketStats(mrrProvider, nhProvider, print) {
  let MiningRigRentals = mrrProvider || mrr;
  let NiceHash = nhProvider || nh;
  let europe_orders;

  try {
    europe_orders = await NiceHash.getOrdersForAlgorithm(0, "scrypt");
  } catch (err) {
  ...
  try {
    american_orders = await NiceHash.getOrdersForAlgorithm(1, "scrypt");
  } catch (err) {
  ...
  try {
    res = await MiningRigRentals.getAlgo("scrypt");
  } catch (err) {
  ...
  try {
    globalStats = await NiceHash.getCurrentGlobalStats24h();
  } catch (err) {
  ...
  try {
    let res = await MiningRigRentals.getAlgo('scrypt');
