  ...
  start() {
    return new Promise((resolve, reject) => {
      ...
      this.upload_xmlhttprequest.addEventListener("load", () => {
        var response = JSON.parse(this.upload_xmlhttprequest.responseText); // Normalize to match regular ipfs-api add output
      ...
      var add_port = true; // Don't add the port if the protocol matches the port
      ...
      if (add_port) built_url += ":" + this.options.port; // Set the upload to use the URL

      this.upload_xmlhttprequest.open("POST", built_url + "/api/v0/add?json=true"); // We are expecting a JSON response

      this.upload_xmlhttprequest.setRequestHeader("accept", "application/json");
      this.upload_xmlhttprequest.setRequestHeader("OIP-Auth", JSON.stringify(this.options.oip_auth)); // Set the encoding type since we are going to be sending FormData

      this.upload_xmlhttprequest.enctype = "multipart/form-data"; // Create a Form to send to the server
