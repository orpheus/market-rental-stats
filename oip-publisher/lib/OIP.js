...
class OIP {
  ...
  getMultiparts(message) {
    ...
    if (message.length <= FLODATA_MAX_LEN) throw new Error("Message can fit in only one Transaction, Multiparts are thus not allowed!");
    let multiparts = [];
    ...
    for (var c in chunks) {
      var mp = new _oipIndex.Multipart();
      ...
      mp.is_valid = mp.isValid().success; // If we are the first multipart, then sign right now, for all others, sign after the first tx has broadcast.
  ...
  async publish(item) {
    let pub_message = {}; // Get the sub-object key

    let publish_type = "register";
    if (item instanceof _oipIndex.Artifact) publish_type = "publish"; // Set it up to be blank

    pub_message[publish_type] = {}; // Set the timestamp of the item

    item.setTimestamp(Date.now());
    let item_json = item.toJSON(); // Remove the version from the returned json, we don't want to publish it twice.

    item_json = item_json.oip042;
    pub_message[publish_type] = item_json; // Create signature preimages

    let signature_preimage;
    if (item instanceof _oipIndex.Artifact) signature_preimage = `${item.getLocation()}-${item.getMainAddress()}-${item.getTimestamp()}`; // if (item instanceof Publisher)
