...
/**
 * A Class used for uploading/adding files to IPFS from Node.js
 */
...
class IPFSAPIAdd {
  ...
  async start() {
    ...
    var onProgressFunc = bytesAdded => {
      var progress_object = {
        bytes_uploaded: bytesAdded
      };

      if (this.options.filesize) {
        var percent = parseFloat((bytesAdded / this.options.filesize * 100).toFixed(2));
        progress_object.upload_progress = percent;
        progress_object.bytes_total = this.options.filesize;
      }

      for (var progress_function of this.progress_subscribers) progress_function(progress_object);
    };

    var response = await this.ipfs.add(file_object, {
